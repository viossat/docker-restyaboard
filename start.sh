...
POSTGRES_ENV_POSTGRES_DATABASE=${POSTGRES_ENV_POSTGRES_DATABASE:-restyaboard}
...
sed -i "s/^.*'R_DB_NAME'.*$/define('R_DB_NAME', '${POSTGRES_ENV_POSTGRES_DATABASE}');/g" /usr/share/nginx/html/server/php/config.inc.php
...
psql -c "CREATE DATABASE ${POSTGRES_ENV_POSTGRES_DATABASE} ENCODING 'UTF8'"
if [ "$?" = 0 ]; then
  psql -d ${POSTGRES_ENV_POSTGRES_DATABASE} -f /usr/share/nginx/html/sql/restyaboard_with_empty_data.sql
fi
set -e
export PGDATABASE=$POSTGRES_ENV_POSTGRES_DATABASE
